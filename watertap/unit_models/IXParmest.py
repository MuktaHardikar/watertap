...
# curve_file =  r'\Users\mhardika\Documents\WaterTap Project Files\IX_Mukta\pfas_summary_files\ix_pfas_curves_MASTER.csv'
curve_file = r'\Users\mhardika\Documents\WaterTap Project Files\IX_Mukta\pfas_summary_files\pfas_all_Mukta_23Oct23.csv'
df_pfas = pd.read_csv(curve_file)
...
resin_fg_dict = {  # functional group
    ...
    "aer1_ground": "Complex Amino",
    "aer1": "Complex Amino",
    "aer2_ground": "Quaternary Amine",
    "aer2": "Quaternary Amine",
    "ira67": "Tertiary Amine",
    "ira410": "Dimethyl Ethanol Amine",
    "a860": "Quaternary Ammonium",
    "ira900": "Trimethyl Ammonium",
    "a592e": "Complex Amino",
    "ix1": "Complex Amino",
    "ix2": "N-Tributyl Amine",
    "aix1_ground": "",
    "a532e": "Complex Amino",
    "calgon_calres_2304": "N-Tributyl Amine" ,
    "d5705": "Unknown",
    "d5706": "Unknown",
    "dowex_psr2+": "N-Tributyl Amine",
    "ira96": "Tertiary Amine",
    "mp62": "Tertiary Amine",
}

resin_type_dict = {
    "a694": "Strong Base Type I Gel Polystyrene-Divinylbenzene",
    "a694e": "Strong Base Type I Gel Polystyrene-Divinylbenzene",
    ...
    "ira67": "Weak Base Gel Polyacrylic",
    "ira410": "Strong Base Type II Gel Polystyrene-Divinylbenzene",
    "a860": "Strong Base Type I Macroporous Polyacrylic-Divinylbenzene",
    "ira900": "Strong Base Type I Macroporous Polystyrene-Divinylbenzene",
    "a592e": "Strong Base Type I Macroporous Polystyrene-Divinylbenzene",
    "ix1": "Strong Base Type I Gel Polystyrene-Divinylbenzene",
    "ix2": "Strong Base Type I Gel Polystyrene-Divinylbenzene",
    "aix1_ground": "",
    "a532e": "Strong Base Macroporous Styrene-Divinylbenzene",
    "calgon_calres_2304": "Strong Base Gel Styrene-Divinylbenzene",
    "d5705": "Weak Base Macroporous Styrene-Divinylbenzene",
    "d5706": "Weak Base Macroporous Styrene-Divinylbenzene",
    "dowex_psr2+": "Strong Base Gel Styrene-Divinylbenzene",
    "ira96": "Weak Base Macroporous Styrene-Divinylbenzene",
    "mp62": "Weak Base Macroporous Styrenic",
}
...
resin_dens_dict = {  # density of resin
    ...
    "aix1_ground": 0.72,
    "a532e": 0.68,
    "calgon_calres_2304": 0.69,
    "d5705":  0.7,  # no data, assumed
    "d5706":  0.7,  # no data, assumed
    "dowex_psr2+": 0.69,
    "ira96": 0.67,
    "mp62": 0.62,
}
...
resin_diam_dict = {  # diameter of resin bead
    ...
    "ix1": 0.096e-3,
    "ix2": 0.096e-3,
    "aix1_ground": 196e-6,  # avg of reported particle size range 180-212 um
    "a532e": 675e-6,
    "calgon_calres_2304": 675e-6,  # no data, assumed
    "d5705": 675e-6,  # no data, assumed
    "d5706": 675e-6,  # no data, assumed
    "dowex_psr2+": 700e-6,
    "ira96": 650e-6,
    "mp62": 470e-6,
}
...
class IXParmest:
        ...
        **kwargs,
        ...
        if fix_initial_guess == dict():
            self.fix_initial_guess = {  # default initial guesses for regressed parameters
                "mass_transfer_coeff": 0.1,  # by default, bv_50 has no initial guess. Default initial guess for bv_50 is avg of BV values from curve.
                "freundlich_n": 1.1,
            }
        else:
    ...
    def plot_estimate_bv50(self):
        ...
        try:
            textstr = "\n".join(
                [
                    f"Guess",
                    f"  k$_T$: {self.fix_initial_guess['mass_transfer_coeff']:.2e}",
    ...
    def get_curve_conditions(self):
        ...
        self.bed_diam = df.bed_diam.unique()[0] * pyunits.m
    ...
    def build_it(self):
        ...
        pf.properties_in[0].flow_mol_phase_comp["Liq", self.target_ion].fix(
            self.c0_mol_flow
        )
        ...
        ix.c_norm[self.target_ion].fix(self.c_norm)
    ...
    def scale_it(self, m=None):
        ...
        constraint_scaling_transform(ix.eq_clark[target_ion], 1e-2)
    ...
    def test_theta(
        ...
        for bv, cnorm in zip(self.test_bvs, self.test_cnorms):
            ...
            ix.c_norm[self.target_ion].fix(cnorm)
            ix.bv.set_value(bv)
    ...
    def plot_theta(self):
        ...
        self.keep_bv_theta = [x for x in self.keep_bv_theta if x !=0]
    ...
    def save_figs(self, overwrite=False, extension=None):
        ...
        for figname, d in self.all_figs.items():
            # fig_file_base = f"figs/{figname}/curve{self.curve}_"
            fig = d["fig"]
            fig_file = f"figs/{figname}/curve{self.curve}_{figname}.png"
            if overwrite:
    ...
    def save_output(self, overwrite=False):
        ...
        theta_file_base = f"theta/curve{self.curve}_theta.csv"

        if hasattr(self, "theta_dict"):
            tmp_dict = dict()
            tmp_dict[self.curve] = self.theta_dict
            df_theta = pd.DataFrame.from_dict(tmp_dict).T
            df_theta["vel_bed"] = self.vel_bed
            df_theta["curve"] = self.curve
    ...
    def _just_plot_curve(self):
        ...
        ax.set_title(
            f"Curve {self.curve}: {self.ref.replace('_', ' ').title()}, {self.compound.upper()}"
        )
...
def main():
    ...
    calc_from_constr = {
        "kinetic_param": "eq_mass_transfer_coeff",
    ...
    c = IXParmest(
        ...
        # just_plot_curve = True,
        # use_this_data=[0,2,3,5,6,7,8],
        )
