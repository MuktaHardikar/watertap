...
def build_wrd_ro_system(blk, prop_package=None, number_trains=3, number_stages=3):
    ...
    blk.product_mixer = Mixer(
        property_package=prop_package,
        inlet_list=[f"train_{i+1}_product" for i in range(number_trains)],
    ...
    for i, n in enumerate(range(number_trains)):
        ...
        prod_mixer_inlet = blk.product_mixer.find_component(f"train_{i+1}_product")
        train.add_component(
            f"train_{i+1}_product_to_mixer",
            Arc(source=train.product.outlet, destination=prod_mixer_inlet),
        )
    ...
    blk.product_mixer_to_permeate = Arc(
        source=blk.product_mixer.outlet, destination=blk.permeate.inlet
    )
...
def add_ro_units(blk, prop_package=None):
    ...
    blk.permeate_mixer = Mixer(
        property_package=prop_package,
        inlet_list=[f"ro_stage_{i}_permeate" for i in range(1, (blk.number_stages + 1))],
        energy_mixing_type=MixingType.extensive,
        momentum_mixing_type=MomentumMixingType.minimize,
    )
    ...
    for t in range(1, blk.number_trains + 1):
        ...
        for s in range(1, (train.number_stages + 1)):
            ...
            pump.efficiency_pump.fix(
                get_config_value(blk.config_data, "pump_efficiency", "pumps", f"pump_{s}")
            )
        ...
        # Set RO configuration for each stage
            ...
            ro_stage.feed_side.channel_height.fix(
                get_config_value(
                    blk.config_data, "channel_height", "reverse_osmosis_1d", f"stage_{s}"
                )
            )
            ro_stage.feed_side.spacer_porosity.fix(
                get_config_value(
                    blk.config_data, "spacer_porosity", "reverse_osmosis_1d", f"stage_{s}"
                )
            )
            ...
            ro_stage.feed_side.length.fix(
                ...
                * get_config_value(
                    blk.config_data, "element_length", "reverse_osmosis_1d", f"stage_{s}"
                )
            )
            ...
            ro_stage.area.fix(
                ...
                * get_config_value(
                    blk.config_data, "number_of_vessels", "reverse_osmosis_1d", f"stage_{s}"
                )
                * get_config_value(
                    blk.config_data,
                    "number_of_elements_per_vessel",
...
def add_ro_connections(blk):
    ...
    for i in range(1, blk.number_stages + 1):
        ...
        blk.add_component(
            ...
            Arc(source=ro_perm, destination=blk.permeate_mixer.find_component(f"ro_stage_{i}_permeate")),
        )
    ...
    blk.permeate_mixer_to_permeate = Arc(
        source=blk.permeate_mixer.outlet, destination=blk.product.inlet
    )
    last_stage = blk.find_component(f"ro_stage_{blk.number_stages}")
    blk.last_stage_to_brine = Arc(source=last_stage.retentate, destination=blk.brine.inlet)
...
def initialize_ro_system(blk):
    ...
    blk.product_mixer.initialize()
    propagate_state(blk.product_mixer_to_permeate)
