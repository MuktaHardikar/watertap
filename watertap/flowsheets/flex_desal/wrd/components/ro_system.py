...
from idaes.models.unit_models import MixingType, MomentumMixingType, Mixer, Separator, Product, Feed, StateJunction
...
def build_wrd_ro_system(blk, prop_package=None):
    ...
    blk.no_of_stages = Param(initialize= 3, mutable=True, doc="Number of RO stages in the system")
...
def set_ro_system_op_conditions(blk):
    ...
    feed_mass_flow_water = get_config_value(blk.config_data,"feed_flow_water", "feed_stream")\
                                *get_config_value(blk.config_data,"feed_density_water", "feed_stream")

    feed_mass_flow_salt = get_config_value(blk.config_data,"feed_conductivity", "feed_stream") \
    ...
    blk.total_ro_feed.inlet.flow_mass_phase_comp[0,"Liq", "H2O"].fix(feed_mass_flow_water)
    blk.total_ro_feed.inlet.flow_mass_phase_comp[0,"Liq", "NaCl"].fix(feed_mass_flow_salt)
...
def build_ro_inlet_stream(blk, test = False):
    ...
    if test:
        ...
        equal_division_factor = get_config_value(blk.config_data,"number_of_trains", "reverse_osmosis_1d")

        feed_mass_flow_water = get_config_value(blk.config_data,"feed_flow_water", "feed_stream")\
                                *get_config_value(blk.config_data,"feed_density_water", "feed_stream")/ equal_division_factor

        feed_mass_flow_salt = get_config_value(blk.config_data,"feed_conductivity", "feed_stream") \
                                *get_config_value(blk.config_data,"feed_conductivity_conversion", "feed_stream")\
                                *get_config_value(blk.config_data,"feed_flow_water", "feed_stream")/ equal_division_factor

        feed_temperature = get_config_value(blk.config_data,"feed_temperature", "feed_stream")
        feed_pressure = get_config_value(blk.config_data,"feed_pressure", "feed_stream")
        ...
        blk.feed.properties[0].flow_mass_phase_comp["Liq", "H2O"].fix(blk.total_ro_feed.properties[0].flow_mass_phase_comp["Liq", "H2O"] / get_config_value(blk.config_data,"number_of_trains", "reverse_osmosis_1d"))
        blk.feed.properties[0].flow_mass_phase_comp["Liq", "NaCl"].fix(blk.total_ro_feed.properties[0].flow_mass_phase_comp["Liq", "NaCl"] / get_config_value(blk.config_data,"number_of_trains", "reverse_osmosis_1d"))
        blk.feed.properties[0].temperature.fix(blk.total_ro_feed.properties[0].temperature)
...
def set_ro_operation_conditions(blk):
    ...
    for i in range(1, (blk.no_of_stages() + 1)):
        ...
        pump.control_volume.properties_out[0].pressure.fix(
            get_config_value(blk.config_data, "pump_outlet_pressure", "pumps", f"pump_{i}")
        )

        pump.efficiency_pump.fix(get_config_value(blk.config_data, "pump_efficiency", "pumps", f"pump_{i}"))
    ...
    for i in range(1,(blk.no_of_stages() + 1)):
        ...
        ro_stage.A_comp.fix(get_config_value(blk.config_data, "A_comp", "reverse_osmosis_1d", f"stage_{i}"))
        ro_stage.B_comp.fix(get_config_value(blk.config_data, "B_comp", "reverse_osmosis_1d", f"stage_{i}"))

        ro_stage.feed_side.channel_height.fix(get_config_value(blk.config_data, "channel_height", "reverse_osmosis_1d", f"stage_{i}"))
        ro_stage.feed_side.spacer_porosity.fix(get_config_value(blk.config_data, "spacer_porosity", "reverse_osmosis_1d", f"stage_{i}"))

        ro_stage.feed_side.length.fix(
            get_config_value(blk.config_data, "number_of_elements_per_vessel", "reverse_osmosis_1d", f"stage_{i}")*
            get_config_value(blk.config_data, "element_length", "reverse_osmosis_1d", f"stage_{i}")
        )
        ...
        ro_stage.area.fix(
            get_config_value(blk.config_data, "element_membrane_area", "reverse_osmosis_1d", f"stage_{i}")*
            get_config_value(blk.config_data, "number_of_vessels", "reverse_osmosis_1d", f"stage_{i}")*
            get_config_value(blk.config_data, "number_of_elements_per_vessel", "reverse_osmosis_1d", f"stage_{i}")
        )

        ro_stage.deltaP.fix(get_config_value(blk.config_data, "pressure_drop", "reverse_osmosis_1d", f"stage_{i}"))

        ro_stage.recovery_mass_phase_comp[0,"Liq", "H2O"].fix(get_config_value(blk.config_data, "water_recovery_mass_phase", "reverse_osmosis_1d", f"stage_{i}"))
...
def add_ro_connections(blk):
    ...
    blk.pump1_to_ro_stage_1 = Arc(source=blk.pump1.outlet, destination=blk.ro_stage_1.inlet)

    # Connect first RO stage to second pump
    blk.ro_stage_1_to_pump2 = Arc(source=blk.ro_stage_1.retentate, destination=blk.pump2.inlet)

    # Connect second pump to second RO stage
    blk.pump2_to_ro_stage_2 = Arc(source=blk.pump2.outlet, destination=blk.ro_stage_2.inlet)

    # Connect second RO stage to third pump
    blk.ro_stage_2_to_pump3 = Arc(source=blk.ro_stage_2.retentate, destination=blk.pump3.inlet)

    # Connect third pump to third RO stage
    blk.pump3_to_ro_stage_3 = Arc(source=blk.pump3.outlet, destination=blk.ro_stage_3.inlet)
    ...
    def constrain_total_ro_product_temperature(blk):
        return blk.total_ro_product.properties[0].temperature == blk.permeate.properties[0].temperature
    
    @blk.Constraint()
    def constrain_total_ro_product_pressure(blk):
        return blk.total_ro_product.properties[0].pressure == blk.permeate.properties[0].pressure
    
    @blk.Constraint()
    def constrain_total_ro_product_flow_mass_phase_comp(blk):
        return blk.total_ro_product.properties[0].flow_vol_phase["Liq"] == blk.permeate.properties[0].flow_vol_phase["Liq"] * get_config_value(blk.config_data,"number_of_trains", "reverse_osmosis_1d")
...
if __name__ == "__main__":
    ...
    print("Permeate production from all trains:", m.fs.ro_train.permeate.properties[0].flow_vol_phase["Liq"]() * get_config_value(m.fs.ro_train.config_data,"number_of_trains", "reverse_osmosis_1d"))

    print("Total Product Stream \n", m.fs.ro_train.total_ro_product.properties[0].display())
    print("Degrees of freedom:", degrees_of_freedom(m))
